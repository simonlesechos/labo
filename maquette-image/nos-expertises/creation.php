<!DOCTYPE html>
<html>

<head>

  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, user-scalable=no">
  <title>Image v4 | Gestion</title>

    <link rel="stylesheet" href="../css/style.css" media="screen" type="text/css" />
</head>

<body class="pagebody-cab">
  <header class="main-header">
<?php include('../includes/header.php'); ?>
  </header>

  <div class="wrapper">

  <div class="header-page">
    <div class="header-page-img">
      <div class="color-filter filter3">
              <div class="titre-page"><h1>Nos expertises</h1><h2 class="bigh2">Des fondamentaux  de la gestion de votre entreprise… à une expertise pointue.</h2></div>
      </div>
    </div>
  </div>
<?php include('../includes/menu-page-expertises.php'); ?>
  <div class="content page-content">
    <div class="max-width">
      <div class="pres-cab">
        <h1 class="bigh1">Accompagnement Création d'entreprise</h1>        
              <iframe width="100%" height="260" style="border: none;" src="../objects/anim-crea/index.html"></iframe>
          <div class="creation-link">
            
          </div>
      </div>    
      <div class="clear"></div>
    </div>
    <div class="actu-page-widget colored-back">
    <h2>Nos expertises</h2>
    <div class="box-actu-container">
        <div class="box-actu">
          <div class="box-actu-img">
            <img src="../img/personnes01.jpg">
          </div>
          <div class="box-actu-excerpt">
            <h4>Comptabilité et fiscal</h4>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu pharetra nec, mattis ac neque. Duis vulputate commodo lectus, ac blandit elit tincidunt id.</p>
          </div>
           <a href="nos-expertises-1.php" class="button">En savoir +</a>
        </div>
        <div class="box-actu">
          <div class="box-actu-img">
            <img src="../img/personnes02.jpg">
          </div>
          <div class="box-actu-excerpt">
            <h4>Social RH</h4>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu pharetra nec, mattis ac neque. Duis vulputate commodo lectus, ac blandit elit tincidunt id.</p>
          </div>
           <a href="nos-expertises-2.php" class="button">En savoir +</a>
        </div> <div class="box-actu">
          <div class="box-actu-img">
            <img src="../img/personnes03.jpg">
          </div>
          <div class="box-actu-excerpt">
            <h4>Gestion </h4>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla quam velit, vulputate eu pharetra nec, mattis ac neque. Duis vulputate commodo lectus, ac blandit elit tincidunt id.</p>
          </div>
           <a href="nos-expertises-3.php" class="button">En savoir +</a>
        </div>
    </div>
  </div>
<?php include('../includes/footer.php');?> 
</div>


</body>

</html>